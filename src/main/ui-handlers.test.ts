...
describe('UI Handlers', () => {
  it('should handle COPY_SELECTION', async () => {
    ...
    expect(commands.handleCopyCommand).toHaveBeenCalledWith({ shouldClose: false });
  });

  it('should handle PASTE_PROPERTY', async () => {
    const msg = { type: 'PASTE_PROPERTY', granules: ['fills'] };
    await handleUIMessage(msg);
    expect(commands.handlePasteCommand).toHaveBeenCalledWith(['fills'], { shouldClose: false });
  });
});
