...
describe('PropertyButton', () => {
  it('should render label', () => {
    render(<PropertyButton label="Fills" granules={['fills']} available={true} onPaste={() => {}} />);
    expect(screen.getByText('Fills')).toBeDefined();
  });

  it('should be disabled if not available', () => {
    render(<PropertyButton label="Strokes" granules={['strokes']} available={false} onPaste={() => {}} />);
    expect(screen.getByText('Strokes')).toHaveProperty('disabled', true);
  });

  it('should call onPaste with granules when clicked', () => {
    const onPaste = vi.fn();
    render(<PropertyButton label="Effects" granules={['effects']} available={true} onPaste={onPaste} />);
